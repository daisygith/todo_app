...
exports.addTask = async function (task) {
    ...
    try {
        ...
        await conn.execute(
            ...
            {description: task.description, status: task.status, finishDate: new Date(task.finishDate)}
        );
        const result = await conn.execute(
            `SELECT * FROM (
                SELECT * FROM task ORDER BY id DESC
            ) WHERE ROWNUM = 1`, [], options
        );
...
exports.updateTask = async function (task) {
    ...
    try {
        ...
        const result = await conn.execute(
            `SELECT * FROM task WHERE id = :id`, [task.id], options
        );
        return result.rows[0];
    } catch (err) {
        console.log('Ouch!', err)
